...
process.title = 'videocity-server';
...
async function run()
{
	...
	await runMediasoupWorkers().then(
		createExpressApp // Create Express app.
	).then(
		runHttpsServer // Run HTTPS server.
	).then(
		runProtooWebSocketServer // Run a protoo WebSocketServer.
	).catch((err) => {
		console.log("server error", err);
	});

	// Log rooms status every hour.
	setInterval(() =>
	{
...
async function createExpressApp()
{
	...
	expressApp.use((req, res, next) =>{
		logger.info("request made [METHOD: %s, URL: %s]", req.method, req.url);
	});
	/**
...
async function runHttpsServer()
{
	...
	{
		logger.error('error on httpsServer: %o', error);
	});
	httpsServer.on('listening', () =>
	{
		...
		await httpsServer.listen(
			Number(config.https.listenPort), config.https.listenIp, () =>
			logger.info('running an HTTPS server listening in %s:%d', config.https.listenIp, config.https.listenPort));
}
